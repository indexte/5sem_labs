...
public class Scheduling {
  ...
  private static List<sProcess> processVector = new ArrayList<>();
  private static Results result = new Results("null","null",0);
  private static String resultsFile = "Summary-Results";
  private static String logsFile = "Summary-Processes";
    ...
    try {
      ...
      while ((line = in.readLine()) != null) {
        ...
        if (line.startsWith("process")) {
          ...
          int userID = Common.s2i(st.nextToken());
          X = Common.R1();
          while (X == -1.0) {
            X = Common.R1();
          }
          ...
          processVector.add(new sProcess(userID, cputime, ioblocking, 0, 0, 0));
        }
  ...
  public static void main(String[] args) {
    ...
    result = SchedulingAlgorithm.run(runtime, logsFile,processVector, result, quantum);
    try {
      ...
      out.println("Process #\tUser\tCPU Time\tIO Blocking\tCPU Completed\tCPU Blocked");
